...
## coef & se containers
ncoef <- length(coef(mod.train[[1]][[1]][[1]]))
ntau <- length(mod.train[[1]][[1]][[1]][['zeta']])
varnames <- c(
  names(coef(mod.train[[1]][[1]][[1]])),
  names(mod.train[[1]][[1]][[1]][['zeta']])
)
...
for(dv in 1:5){               ## extract & avareaged coefficients
  for(j in 1:10){
  coefList[[dv]][, j]  <- rowMeans(
    sapply(
      mod.train[[dv]][[j]], function(x){ c(coef(x), x[['zeta']]) }
    )
  )
  }
}
...
for(i in unique(pdta[, 'depVar'])){
  ...
  ggsave(
    plot = p, 
    file = paste0('./out/figCoefPlotKfold_', i, '.pdf'),
