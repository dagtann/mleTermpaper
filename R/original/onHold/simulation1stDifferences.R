...
for(i in erDvs){                   ## for all dependent variables
  ...
  dta[dta[, 'dv'] == i, 'c2'] <- plogis(tau[, 1] - pred.0) - plogis(tau[, 1] - pred.4)
  dta[dta[, 'dv'] == i, 'c3'] <- plogis(tau[, 2] - pred.0) - plogis(tau[, 1] - pred.0) - 
    plogis(tau[, 2] - pred.4) - plogis(tau[, 1] - pred.4)
  dta[dta[, 'dv'] == i, 'c4'] <- plogis(tau[, 3] - pred.0) - plogis(tau[, 2] - pred.0) - 
      plogis(tau[, 3] - pred.4) - plogis(tau[, 2] - pred.4)
  dta[dta[, 'dv'] == i, 'c5'] <- plogis(tau[, 4] - pred.0) - plogis(tau[, 3] - pred.0) - 
      plogis(tau[, 4] - pred.4) - plogis(tau[, 3] - pred.4)
  dta[dta[, 'dv'] == i, 'c6'] <- plogis(tau[, 5] - pred.0) - plogis(tau[, 4] - pred.0) - 
      plogis(tau[, 5] - pred.4) - plogis(tau[, 4] - pred.4)
  dta[dta[, 'dv'] == i, 'c7'] <- (1 - plogis(tau[, 5] - pred.0)) - 
...
sub.label <- textGrob(                 
  'Per cent point change in probability of category given maximal change in co-optation; 10,000 iterations.', 
