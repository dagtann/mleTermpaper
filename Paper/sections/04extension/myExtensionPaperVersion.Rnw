...
myTerms <- c(       ## drop vars that are not meaningful controls
  "lead_fh1", "cooptation", "flip_ciri_phys", 
...
form <- formula(                         ## generic model formula
  I(as.numeric(lead_fh1)-1) ~ 
  cooptation_discrete + flip_ciri_phys +  ## core
...
form <- formula(                         ## generic model formula
  I(as.numeric(lead_fh1)-1) ~ cooptation_discrete + flip_ciri_phys +  ## core
  gled_lgdppc + gled_lpop +           ## socio-economic structure
  geddes_personal + geddes_monarch + geddes_party +  ## political
...
library('stargazer')
fit.empty <- update(fit, . ~ . - cooptation_discrete:flip_ciri_phys)
stargazer(
  fit.empty, fit, style = 'apsr',
  se = list(
    sqrt(diag(vcovHAC(fit.empty))), 
    sqrt(diag(vcovHAC(fit)))
  ),
  title = 'Extended model regression results',
  label = 'tbl:olsExtension',
  dep.var.labels = c('ER$_{t+1}$'),
  covariate.labels = c(
    "Co-optation: 1. Either-or",
    "Co-optation: 2. Legislature, 1 party",
    "Co-optation: 3. Legislature, mult. parties",
    "Current PI",
    "log(GDP per capita)",
    "log(Population)",
    "Personalist regime",
    "Monarchy",
    "Dominant party",
    "Past leadership failure",
    "Past coup attempts",
    "log(Civil war)",
    "log(Interstate war)",
    "log(General strike)",
    "log(Riots)",
    "log(Antigovernment demonstr.)",
    "Year-1989",
    "(Year-1989)$^2$",
    "Co-optation 1*Current PI",
    "Co-optation 2*Current PI",
    "Co-optation 3*Current PI"
  ),
  font.size = 'scriptsize',
  single.row = TRUE, digits = 2, ci = FALSE, summary = FALSE,
  out = file.path(pathPaper, 'sections/appendix/extensionTable.tex')
)
## END
@